...
from utils import compute_metrics_task1,compute_metrics_task2,compute_metrics_task3
...
def collate_fn(data):
    ...
    for x in data:
        ...
        for e in entity:
            id = tokenizer('{}是[MASK]，'.format(e,turn[x['entity'][e]])).input_ids[1:-1]
            input_id += id
            label+= [0]*(len(id)-1)
            label += turn[x['entity'][e]]
...
import time
def train_model(model, fgm,pgd,train_loader,epoch):
    model.train()
    ...
    for step, (input_ids, attention_mask,y) in enumerate(tk):
        ...
        """
        fgm.attack()  # FGM对抗扰动
        output2 = model(input_ids=input_ids, input_mask=attention_mask,  labels=y)
        ...
        logits[:,115271:125883]=0
        logits[:,125884:]=0
        loss2 = criterion(logits,output['labels'])
    ...
    with torch.no_grad():
        ...
        for idx, (input_ids, attention_mask, y) in enumerate(tk):
            ...
            for i in output['labels']:
                golds.append({125883:0,115270:1,68158:2,110093:3,64243:4}[i.cpu().item()])
...
from DeBERTa.apps.models.masked_language_model import MaskedLanguageModel
model = MaskedLanguageModel.load_model(CFG['model'].replace("/", "-")+'/pytorch_model.bin',CFG['model'].replace("/", "-")+'/config.json')  # 模型
...
for epoch in range(CFG['epochs']):
    ...
    preds = []
    for i in range(len(pred)):
        if np.array(pred[i]).argmax(0) == y_truth[i]:
            preds.append(1)
        else:
            preds.append(0)
    truth = [1] * len(preds)

    log_1,l1 = compute_metrics_task1(preds,truth)
    log(log_1,path)
    log_2,l2 = compute_metrics_task2(np.array(pred).argmax(1),y_truth)
    log(log_2,path)
    log_3,l3 = compute_metrics_task3(np.array(pred).argmax(1),y_truth)
    log(log_3,path)

    model_name = path+'/{}_{}_model'.format(epoch,str(l1['F1']),str(l2['F1']),str(val_acc))
    os.mkdir(model_name)
